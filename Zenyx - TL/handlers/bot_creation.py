...
async def handle_token_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        user_data = await redis_service.get_user_data(user_id)
        if not user_data:
            # Criar um novo registro de usuário se não existir
            user_data = {
                'id': user_id,
                ...
                'referrals': [],
                'is_admin_vip': False,
                'last_activity': update.message.date.isoformat()
            }
        ...
        # Notificar que o bot foi criado com sucesso
        await processing_message.edit_text(
...
async def show_message_preview(query, context, bot_data):
    ...
    if config.get('media_id') and config.get('media_type'):
        try:
            ...
            logger.error(f"Erro ao enviar preview com mídia: {e}")
